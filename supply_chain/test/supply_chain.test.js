...
contract('SupplyChain', function(accounts) {
    ...
    it("should add an item with the provided name and price", async() => {
        ...
        */
        ...
        let tx = await supplyChain.addItem(name, price, {from: alice})
        var eventSku = tx.logs[0].args.sku
        eventSku = eventSku.toNumber()
       ...
       assert.equal(result[1], eventSku, 'the event information and item information don\'t match')
        assert.equal(result[2].toNumber(), price, 'the price of the last added item does not match the expected value')
        assert.equal(result[3].toNumber(), 0, 'the state of the item should be "For Sale", which should be declared first in the State Enum')
    ...
    it("should allow someone to purchase an item", async() => {
        ...
        await event.watch((err, res) => {
            sku = res.args.sku.toString(10) //sku=0
            eventEmitted = true
        })

        const amount = web3.toWei(2, "ether") //msg.value takes input in wei.
        ...
        assert.equal(result[3].toNumber()/*toString(10)*/, 1, 'the state of the item should be "Sold", which should be declared second in the State Enum')
        ...
        assert.equal(aliceBalanceAfter, aliceBalanceBefore + price, "alice's balance should be increased by the price of the item")
